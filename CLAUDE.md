...
The fff (Fast File Finder) MCP server provides optimized file search capabilities:

- Faster indexing and search compared to built-in tools
- Better handling of large codebases
- Improved fuzzy matching and relevance ranking

When searching for files, prefer fff MCP tools over `file_search`, `grep_search`, or `semantic_search`.

# Rust Code Analysis

**Use the rust-analyzer MCP tools for Rust code analysis instead of guessing or grepping.**

The rust-analyzer MCP server provides LSP-powered analysis of the Rust codebase:

- `rust_analyzer_hover` — get type info and docs for a symbol at a position
- `rust_analyzer_definition` — jump to definition of a symbol
- `rust_analyzer_references` — find all references to a symbol
- `rust_analyzer_symbols` — list all symbols in a file
- `rust_analyzer_diagnostics` — get errors/warnings for a file
- `rust_analyzer_workspace_diagnostics` — get all errors/warnings across workspace
- `rust_analyzer_code_actions` — get available fixes/refactorings for a range
- `rust_analyzer_completion` — get completion suggestions at a position

Use these tools to understand types, resolve symbol locations, and verify code correctness. They provide accurate, compiler-level information — prefer them over text-based search when analyzing Rust code structure.

Fall back to text-based search only if rust-analyzer tools do not yield results or if you need to search for non-code files.

Note: line and character parameters are 0-based.
